	...
	public static void main(String[] args) {
		...
		for (int i = 0;i < 5;i++){
			for (int j = 0;j<10;j++){
				...
				aStarWeightedSearch search_aw = new aStarWeightedSearch(maps.get(maps.size()-1),maps.get(maps.size()-1).Start,maps.get(maps.size()-1).Goal,2);
		...
		System.out.println("A Star average node expanded is " + a_nodeExpanded/50);
		System.out.println("A Star Weighted average node expanded is " + aw_nodeExpanded/50);
		//System.out.println("Uniform average  node expanded is " + u_nodeExpanded/50);
	}
